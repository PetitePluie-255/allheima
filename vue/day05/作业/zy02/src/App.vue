<template>
  <div>
    <div>
      ...
      <input id="orderNo"  v-model.number="num" type="number"/>
      <button @click="check">查询</button>
    </div>
    <div class="logistics-info" v-show="isshow">
   <HappGoods v-show="isshow">
      <template #SunFen >
        {{ shun }}
      </template>
      <template #name>
   ...
   <h2>物流相关信息</h2>
   <HaWhere v-show="isshow" :logisticsInfo="logisticsInfo" :user="user">
    <template #status >
      <p v-if="status===0" v-color="color">未发货</p>
      <p v-else-if="status===1" v-color="color1">已发货</p>
      <p v-else-if="status===2" v-color="color2">已签收</p>
      <p v-else-if="status===3" v-color="color3">未签收</p>
    </template>
   </HaWhere>
    </div>
  </div>
...
<script>
import HaWhere from "./components/HaWhere.vue"
import HappGoods from "./components/HappGoods.vue"
   export default {
    components:{
      HappGoods,
      HaWhere
    ...
    directives:{
      ...
      methods:{
        check(){
          if(!this.num) return
          this.isshow=true
          this.shun=this.orderInfo.logisticsNo
          this.name=this.orderInfo.goodInfo.goodName
          this.status=this.orderInfo.logisticsStatus
          this.logisticsInfo=this.orderInfo.logisticsInfo
          this.user=this.orderInfo.role
